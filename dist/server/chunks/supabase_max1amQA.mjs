...
import { an as __toESM, rn as __commonJSMin } from "./runtime_zVK3H1hm.mjs";
	...
	while (true) {
		...
		try {
			...
			if (retryEnabled && attemptCount < DEFAULT_MAX_RETRIES) {
				...
				attemptCount++;
				await sleep$1(delay, request.signal);
				continue;
			}
			throw fetchError;
		}
...
var serialize$1 = (operator, value) => {
	if (operator === "in") {
...
var RealtimePostgresFilterBuilder = class {
	...
	add(column, operator, value, negate = false) {
		...
		this.filters.push(`${column}=${prefix}${serialize$1(operator, value)}`);
		return this;
	}
...
var DEFAULT_HEADERS$1 = { "X-Client-Info": `storage-js/2.116.0` };
...
* character.
*/
var TO_BASE64URL$1 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_".split("");
/**
* An array of characters that can appear in a Base64-URL encoded string but
* should be ignored.
*/
var IGNORE_BASE64URL$1 = " 	\n\r=".split("");
/**
* An array of 128 numbers that map a Base64-URL character to 6 bits, or if -2
* used to skip the character, or if -1 used to error out.
*/
var FROM_BASE64URL$1 = (() => {
	const charMap = new Array(128);
	for (let i = 0; i < charMap.length; i += 1) charMap[i] = -1;
	for (let i = 0; i < IGNORE_BASE64URL$1.length; i += 1) charMap[IGNORE_BASE64URL$1[i].charCodeAt(0)] = -2;
	for (let i = 0; i < TO_BASE64URL$1.length; i += 1) charMap[TO_BASE64URL$1[i].charCodeAt(0)] = i;
	return charMap;
})();
/**
...
function byteToBase64URL(byte, state, emit) {
	if (byte !== null) {
		...
		while (state.queuedBits >= 6) {
			emit(TO_BASE64URL$1[state.queue >> state.queuedBits - 6 & 63]);
			state.queuedBits -= 6;
		}
...
var GoTrueClient = class GoTrueClient {
	...
	*/
	...
	async _exchangeCodeForSession(authCode, options) {
		...
		const requestedFlowId = hasExplicitFlowId ? validatePKCEFlowId(options === null || options === void 0 ? void 0 : options.flowId) : isBrowser$1() ? validatePKCEFlowId(parseParametersFromURL(window.location.href)[PKCE_FLOW_ID_PARAM]) : null;
	...
	*/
	async signInWithSSO(params) {
		...
		try {
			...
			if (((_c = result.data) === null || _c === void 0 ? void 0 : _c.url) && isBrowser$1() && !((_d = params.options) === null || _d === void 0 ? void 0 : _d.skipBrowserRedirect)) window.location.assign(result.data.url);
...
* ```
*/
var createClient$1 = (supabaseUrl, supabaseKey, options) => {
...
function parseCookieHeader(header) {
	const parsed = import_dist.parse(header);
...
function createStorageFromOptions(options, isServerClient) {
	...
	setAll = async (setCookies, headers) => {
		...
		if (shouldSendHeaders) hasSentHeaders = true;
	};
	return {
		getAll,
		setAll,
...
function createServerClient(supabaseUrl, supabaseKey, options) {
	...
	const client = createClient$1(supabaseUrl, supabaseKey, {
		...options,
		global: {
			...options?.global,
			headers: {
...
function createClient(requestHeaders, cookies) {
